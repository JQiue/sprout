...
pub async fn create_site(
  ...
  match deploy_source {
    DeploySource::Manual => setup_manual_management(deployment, &site, &state.db).await,
    DeploySource::Template => {
...
async fn setup_manual_management(
  ...
  deployment::ActiveModel {
    ...
    upload_token: Set(Some(init_response.data.upload_token.clone())),
    ..Default::default()
  }
  .update(db)
  .await
  .map_err(|_| StatusCode::DbError)?;
  Ok(json!({
      "upload_url": init_response.data.upload_token,
